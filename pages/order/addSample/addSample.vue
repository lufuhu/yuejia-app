<template>
	<view class="u-p-40">
		<u-form :model="form" :rules="rules" :label-width="140" ref="uForm">
			...
			<u-form-item label="产品规格" prop="specification">
				<u-input placeholder="请输入产品规格" v-model="form.specification" type="text"></u-input>
			</u-form-item>
			<u-form-item label="产品数量" prop="num">
				<u-input placeholder="请输入产品数量" v-model="form.num" type="text"></u-input>
			</u-form-item>
			<u-form-item label="产品单价" prop="price">
				<u-input placeholder="请输入产品单价" v-model="form.price" type="text"></u-input>
			</u-form-item>
			<u-form-item label="额外费用" prop="carriage">
				<u-input placeholder="请输入额外费用(顺丰等)" v-model="form.carriage" type="text"></u-input>
			</u-form-item>
		</u-form>
		...
		methods: {
			submit() {
				this.$refs.uForm.validate(valid => {
					if (valid) {
						this.$u.post('/api/v1/order/sample', this.form).then(res => {
							uni.showToast({
							    title: '提交成功',
							    duration: 1600
							});
